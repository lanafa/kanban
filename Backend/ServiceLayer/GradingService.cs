...
namespace IntroSE.Kanban.Backend.ServiceLayer
{
    public class GradingService
    {
        ...
        ColumnSercive columnSercive=new ColumnSercive();
        ...
        {
            return columnSercive.LimitColumn(email, boardName, columnOrdinal, limit);
        ...
        public string GetColumnLimit(string email, string boardName, int columnOrdinal)
        {
            try
            {
                if (boardService.GetBoardController().GetBoard(email, boardName) == null)
                {
                    JsonSerializerOptions options = new JsonSerializerOptions();
                    options.WriteIndented = true;
                    options.DefaultIgnoreCondition = System.Text.Json.Serialization.JsonIgnoreCondition.WhenWritingNull;
                    string json = System.Text.Json.JsonSerializer.Serialize(new Response { ErrorMessage = "board not exist", ReturnValue = null }, options);
                    return json;
                }
                else
                {
                    int limit = boardService.GetBoardController().GetBoard(email, boardName).getColumnLimit(columnOrdinal);
                    JsonSerializerOptions options = new JsonSerializerOptions();
                    options.WriteIndented = true;
                    options.DefaultIgnoreCondition = System.Text.Json.Serialization.JsonIgnoreCondition.WhenWritingNull;
        ...
        public string GetColumnName(string email, string boardName, int columnOrdinal)
        {
            try
            {
                string name = boardService.GetBoardController().GetBoard(email, boardName).getColumnName(columnOrdinal);
                JsonSerializerOptions options = new JsonSerializerOptions();
                options.WriteIndented = true;
                options.DefaultIgnoreCondition = System.Text.Json.Serialization.JsonIgnoreCondition.WhenWritingNull;
        ...
        public string GetColumn(string email, string boardName, int columnOrdinal)
        {
            ...
            try
            {
                if (boardService.GetBoardController().GetBoard(email, boardName) == null)
                {
                    Response response = new Response { ErrorMessage = "board not exist", ReturnValue = null };
                    return System.Text.Json.JsonSerializer.Serialize(response, options);
                }
                LinkedList<Task> tasks = boardService.GetBoardController().GetBoard(email, boardName).GetColumn(columnOrdinal);
                TaskJson[] taskJsons = new TaskJson[tasks.Count];
                for (int i = 0; i < tasks.Count; i++)
                {
                    TaskJson taskJson = new TaskJson();
                    taskJson.Id = tasks.ElementAt(i).getID();
                    taskJson.CreationTime = tasks.ElementAt(i).getCreationTime();
                    taskJson.Title = tasks.ElementAt(i).getTitle();
                    taskJson.Description = tasks.ElementAt(i).getDescription();
                    taskJson.DueDate = tasks.ElementAt(i).getDueDate();
        ...
        {return columnSercive.InProgressTasks(email);
        }
    }
